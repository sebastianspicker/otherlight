...
it("invalidates exact transit reference epochs when V4 orbit geometry changes on the same config object", async () => {
  const cfg: SimulationConfigV4 = {
    ...
    bodies: {
      stars: [
        { id: "star-a", r: 1, m: 1, teffK: 6200, loggCgs: 4.2, passband: "g" },
        { id: "star-b", r: 0.9, m: 0.5, luminosityScale: 0 },
      ],
      planets: [
        {
          id: "planet-1",
    ...
    orbits: {
      binary: { a: 5, e: 0, inc: Math.PI / 2, Omega: 0, omega: 0, period: 1000, t0: 0 },
      hierarchy: [{ childId: "planet-1", parentId: "star-a", relation: "orbits" }],
    },
    photometry: { baselineFlux: 1, limbDarkeningModel: { bandpass: "g" } },
  };

  void stepNativeSimulationV4({
    config: cfg,
    tObsSec: 0,
    mode: "realtime",
    executionMode: "scientific-browser",
  });
  cfg.bodies.planets[0].orbit.omega = 0.8;

  const reused = stepNativeSimulationV4({
    config: cfg,
    tObsSec: 0,
    mode: "realtime",
    executionMode: "scientific-browser",
  }).step;
  const fresh = stepNativeSimulationV4({
    config: JSON.parse(JSON.stringify(cfg)) as SimulationConfigV4,
    tObsSec: 0,
    mode: "realtime",
    executionMode: "scientific-browser",
  }).step;

  expect(reused.timing?.planetTransitCenterSec).toBeCloseTo(fresh.timing?.planetTransitCenterSec ?? 0, 8);
  expect(reused.timing?.planetTtvSec).toBeCloseTo(fresh.timing?.planetTtvSec ?? 0, 8);
});
...
it("rejects semantically invalid direct V4 orbits in scientific-browser mode", () => {
  ...
  expect((caught as ScientificBrowserRuntimeError).details[0]).toContain("orbits.binary.e must be in [0, 1)");
});
