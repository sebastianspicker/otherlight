...
function bodyChordLine(
  geometry: RenderOcculterGeometry | undefined,
  rStar: number,
  label: string,
  color: string,
): SceneLine | undefined {
...
export function sceneChordLines(
  ...
  moon: RenderOcculterGeometry | undefined,
): SceneChordLines {
...
export function sceneVisibilityBadges(step: SimulationFrame): LightCurveBadge[] {
  return [
...
export function sceneFeatureBadges(params: BrowserScenarioDraft): LightCurveBadge[] {
...
function clockOffsetBadge(step: SimulationFrame): LightCurveBadge | undefined {
...
function einsteinDelayBadge(step: SimulationFrame): LightCurveBadge | undefined {
...
function closeEncounterBadge(step: SimulationFrame): LightCurveBadge | undefined {
  return step.physicsDiagnostics.closeEncounterFlags.length > 0
    ? { label: "close encounter warning", color: "#ef476f" }
    : undefined;
}

export function scenePhysicsBadges(step: SimulationFrame): LightCurveBadge[] {
