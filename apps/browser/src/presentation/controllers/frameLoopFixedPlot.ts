...
import { createInstrumentNoiseState } from "../../domain/photometry/instrumentNoise";
...
function fixedPlotDurationSec(timing: SimulationFrame["timing"]): number {
  return Math.max(
...
function fixedPlotEventExtentSec(timing: SimulationFrame["timing"]): number {
  return Math.max(
...
export function rebuildFixedPlot(args: {
  ...
  const stepPreview: Array<{ t: number; step: SimulationFrame }> = [];
  const times: number[] = [];
