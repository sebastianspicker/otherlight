...
} from "../../src/domain/photometry/instrumentNoise";
import {
...
import {
  ...
  circleOverlapArea,
} from "../../src/domain/simulation/v4/nativePhotometry";
import {
...
} from "../../src/application/productViewState";
import { WORKSPACE_SCHEMA_VERSION } from "../../src/infrastructure/workspace/workspaceDocument";
...
const repoRoot = existsSync(path.join(process.cwd(), "contracts"))
  ? process.cwd()
  : path.resolve(process.cwd(), "../..");
const manifest = JSON.parse(
  readFileSync(path.join(repoRoot, "contracts/capabilities-v1/manifest.json"), "utf8"),
) as {
    ...
    for (const capability of manifest.capabilities) {
      ...
      for (const platform of [capability.website, capability.macos, capability.iphone, capability.ipad]) {
        ...
        for (const evidence of platform.evidence)
          expect(existsSync(path.join(repoRoot, evidence)), evidence).toBe(true);
      }
    }
  });
