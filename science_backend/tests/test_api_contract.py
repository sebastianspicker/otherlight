...
def test_http_result_manifest_round_trips_epoch_versions_g_and_tolerances(
    ...
    monkeypatch.setattr(api, "run_forward", fake_run)
    monkeypatch.setattr(api, "_write_arrow", fake_arrow_writer())
...
def test_cancellation_prevents_success_and_artifact_publication(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
) -> None:
    started = Event()
    ...
    monkeypatch.setattr(api, "run_forward", blocking_run)
    monkeypatch.setattr(
        api,
        "_write_arrow",
        lambda _result, _root, **_kwargs: artifact_writes.append(True) or "a" * 64,
    )
    service = api.V5ApiService(tmp_path)
...
def test_wall_deadline_is_reported_as_exhausted_work_budget(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
) -> None:
    artifact_writes: list[bool] = []
    ...
    monkeypatch.setattr(api, "_capability_snapshot", available_capabilities)
    monkeypatch.setattr(api, "MAX_FORWARD_WALL_TIME_SECONDS", 0)
    monkeypatch.setattr(api, "run_forward", deadline_checking_run)
    monkeypatch.setattr(
        api,
        "_write_arrow",
        lambda _result, _root, **_kwargs: artifact_writes.append(True) or "a" * 64,
...
def test_service_rejects_work_above_outstanding_capacity(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
) -> None:
    started = Event()
    ...
    monkeypatch.setattr(api, "run_forward", blocking_run)
    service = api.V5ApiService(tmp_path, max_outstanding_jobs=1)
...
def test_service_evicts_oldest_terminal_jobs_at_retention_limit(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
) -> None:
    def fake_run(request: ForwardRunRequest, *, cancel_requested=None):
    ...
    monkeypatch.setattr(api, "run_forward", fake_run)
    monkeypatch.setattr(api, "_write_arrow", fake_arrow_writer())
    service = api.V5ApiService(tmp_path, max_terminal_jobs=2)
...
def test_shared_contract_fixture_rejects_four_body_request_as_invalid_contract(
    ...
    TestClient = test_client_or_skip()
...
def test_lifespan_closes_only_the_service_it_created(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    TestClient = test_client_or_skip()
    ...
    monkeypatch.setattr(api, "V5ApiService", RecordingService)
