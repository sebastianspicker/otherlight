...
export type DebugOverlayToggles = {
  ...
  /** Show impact parameter proxies b ≈ |y|/R* (from step.meta.bPlanet/bMoon). */
...
export function drawObserverGizmoInset(
  ...
  ctx.font = th.font;
  ctx.fillText("Observer position", 56, 18);
  ctx.font = th.fontSmall;
  ctx.fillText("star → observer (you)", 56, boxH - 36);
...
export function drawDebugOverlayV3(
  ...
  if (dbg.showObserverDir) {
    drawObserverGizmoInset(ctx, size, observerDir, th);
  }

  // Main-view observer marker (purely visual) can be drawn by the caller *before* bodies,
  // but allowing it here as well keeps this module self-contained.
  if (dbg.showObserverMarker) {
    drawObserverMarkerMainView(ctx, size, observerDir, { label: "Observer" });
  }
