...
export function estimateTransitEvent(args: {
  ...
  if (Number.isFinite(periodSec) && (periodSec as number) > 0 && Number.isFinite(t0Sec)) {
    // Use floor-based epoch tracking to avoid discontinuous jumps.
    // Math.round causes a jump at the half-period boundary; Math.floor
    // ensures the epoch number increases monotonically with time.
