...
function buildPhysicsDiagnostics(
  ...
  const ltteEnabled = Boolean(rel?.enabled && rel?.ltte !== false);
  const shapiroEnabled = Boolean(rel?.enabled && rel?.shapiro !== false);
      ...
      status: !ltteEnabled
        ...
        : timingConvergence
          ...
          : ltteValue !== undefined
            ...
            : "unavailable",
      valueSec: ltteValue,
      validityFlags: timingConvergence?.validityFlags,
    },
    shapiroConvergence: {
      enabled: shapiroEnabled,
      status: !shapiroEnabled
        ? "disabled"
        : timingConvergence
          ? timingConvergence.usedShapiro && timingConvergence.converged
            ? "ok"
            : "unavailable"
          : shapiroValue !== undefined
            ? "ok"
            : "unavailable",
      valueSec: shapiroValue,
      validityFlags: timingConvergence?.usedShapiro ? timingConvergence.validityFlags : undefined,
    },
    integratorStats: {
      mode,
      nbodyEnabled,
...
function mapStepToV3(
  ...
  return {
    ...
    debug: {
      ...
      displayFluxValue: step.meta?.displayFluxValue,
      stellarVariabilityFlux: step.meta?.stellarVariabilityFlux,
      eventTimingSolvePlanet: step.meta?.eventTimingConvergence?.planet,
      eventTimingSolveMoon: step.meta?.eventTimingConvergence?.moon,
    },
  };
}
