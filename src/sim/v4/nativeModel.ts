...
export function buildNativeSnapshot(config: SimulationConfigV4, tObsSec: number): NativeSnapshot {
  ...
  const lumBraw = Math.max(
    0,
    finiteOrDefault(starB.luminosityScale, config.mode === "detached-binary-lab" ? 0.3 : 0),
  );
...
export function computeFluxBundle(
  ...
  const baselineStellar = luminousStars.reduce((s, x) => s + x.luminosity, 0);
  ...
  const transitFactor = baselineStellar > 0 ? clamp01(stellarFromEclipses / baselineStellar) : 1;
