...
function hasFinitePositiveBodyRadius(body: unknown): boolean {
  return (
    !!body &&
    typeof body === "object" &&
    Number.isFinite((body as { r?: unknown }).r) &&
    (body as { r: number }).r > 0
  );
}

function hasFiniteOrbitSemantics(orbit: unknown): boolean {
  if (!orbit || typeof orbit !== "object") return false;
  const candidate = orbit as { a?: unknown; period?: unknown };
  return (
    Number.isFinite(candidate.a) &&
    (candidate.a as number) > 0 &&
    Number.isFinite(candidate.period) &&
    (candidate.period as number) > 0
  );
}

function isValidReferenceInitConfig(config: unknown): config is SimulationConfigV4 {
  if (!isSimulationConfigV4(config)) return false;
  const candidate = config as SimulationConfigV4;
  if (!candidate.bodies.stars.every(hasFinitePositiveBodyRadius)) return false;
  if (
    !candidate.bodies.planets.every(
      (planet) => hasFinitePositiveBodyRadius(planet) && hasFiniteOrbitSemantics(planet.orbit),
    )
  )
    return false;
  if (
    !candidate.bodies.moons.every(
      (moon) => hasFinitePositiveBodyRadius(moon) && hasFiniteOrbitSemantics(moon.orbit),
    )
  )
    return false;
  return hasFiniteOrbitSemantics(candidate.orbits.binary);
}
...
self.addEventListener("message", async (ev: MessageEvent<RequestMsg>) => {
  ...
  try {
    ...
    if (msg.kind === "init") {
      if (!isValidReferenceInitConfig(msg.config)) {
        throw new Error("referenceWorker: invalid init config.");
      }
