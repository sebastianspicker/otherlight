 ...
 *   F_total = (baseline + phaseCurve + forwardScattering + variability) * F_transit
 */
export function forwardScatteringFlux(params: ForwardScatteringFluxParams): number {
  ...
  if (kind === "gaussian-time") {
    ...
    // when offset != 0. This is intentionally simple; for real models you'd use orbital geometry.
    const offset = isFiniteNumber(model.phaseOffset) ? model.phaseOffset : 0;
    const cosTheta = Math.cos(Math.acos(cosTheta0) + offset);

    const g = isFiniteNumber(model.g) ? model.g : 0.8;

    // HG phase function has strong divergence for g->1 at cosθ->1; in reality, finite star size
    // and multiple scattering smear that peak. We include a softening by blending with a Gaussian
    // in scattering angle around θ=0, with width sigmaPhase.
    const theta = Math.acos(clamp(cosTheta, -1, 1));
    const soften = Math.exp(-(theta * theta) / (2 * sigmaClamped * sigmaClamped));

    // Use HG as a shape factor and renormalize it relative to its value at θ=0,
    // so amp roughly corresponds to near-maximum brightening scale.
    const p = henyeyGreensteinPhase(g, Math.cos(theta));
    const p0 = henyeyGreensteinPhase(g, 1);

    const shape = p0 > 0 ? (p / p0) : 0;

    f = amp * shape * soften;
  }

  if (!Number.isFinite(f)) return 0;
  if (clampNonNeg) return Math.max(0, f);
  return f;
}
