...
Then:
  ...
  r_planet = r_bary - (m_m / (m_p + m_m)) * r_rel
...
For each body (relative position r_rel, velocity v_rel):
  ...
  a_GR = (mu_star / (c^2 * r^3)) * [ (4*mu_star/r - v_rel^2) * r_rel + 4*(r_rel dot v_rel) * v_rel ]
...
When not in N-body mode, a per-orbit precession is derived from:
  ...
  delta_omega = 6*pi*mu / (a * (1 - e^2) * c^2)
...
Three-parameter (reduced Claret):
  ...
  I(mu) = 1 - a1 * (1 - mu^0.5) - a2 * (1 - mu) - a3 * (1 - mu^1.5)
...
The attenuated stellar flux is:
  ...
  F = (int I(x,y) * P(x,y) * T_total(x,y) dA) / (int I(x,y) * P(x,y) dA)
...
The variability model is:
  ...
  f_var = C + A_beam * sin(phi + phi_b) + A_ellip * cos(2*(phi + phi_e))
...
- OU / AR(1): x_{t+dt} = x_t * exp(-dt/tau) + sigma * sqrt(1 - exp(-2dt/tau)) * N(0,1)
- 1/f bank: sum of OU components with log-spaced taus
- Random walk: x_{t+dt} = x_t + sigma_rw * sqrt(dt) * N(0,1)

Photon noise:

Flux is converted to electrons:

  N_e = max(0, F * throughput * electronsPerUnitFlux * exposureSec)
